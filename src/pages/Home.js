  ...
  return (
    <div data-testid="home-page">
      ...
      <section className="relative max-w-7xl mx-auto px-6 lg:px-10 pt-16 md:pt-24 pb-16">
        ...
        >
          ...
          <h1 className="font-serif text-5xl md:text-7xl text-forest-900 leading-[1.05] tracking-tight">
            Trends worth slowing down for — across travel, tech, finance, products, sports, and markets.
          </h1>
          ...
          <div className="grid lg:grid-cols-12 gap-6 lg:gap-8">
            <Link to={`/post/${hero.slug}`} className="lg:col-span-8 group flex flex-col overflow-hidden bg-white border border-sand-300 hover:border-sage/40 transition-all duration-300 hover:-translate-y-1">
              <div className="overflow-hidden aspect-[16/10] bg-sand-200">
                <img src={hero.cover_image} alt={hero.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
              </div>
              <div className="p-8 md:p-10 flex flex-col flex-1">
                <div className="eyebrow mb-4 capitalize">{hero.category} · Featured</div>
                ...
                <div className="mt-auto pt-6 text-xs text-forest-500 flex gap-3 items-center">
                  <span>{hero.author}</span><span className="opacity-50">·</span><span>{hero.read_time || 7} min read</span>
                </div>
              </div>
            </Link>
      ...
      <section className="max-w-7xl mx-auto px-6 lg:px-10 my-10">
        <AdSlot size="banner" label="Top-of-page banner — 970×250" />
      </section>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-5 lg:gap-6">
          {categories.map(c => (
            ...
            >
              ...
              <div className="text-sm text-forest-500 mt-auto pt-4">{c.count} essays</div>
            </Link>
          ))}
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-6 lg:px-10 py-12">
        ...
        <div className="my-12"><AdSlot size="in" label="In-feed native ad — 728×140" /></div>
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
...
function NewsletterForm() {
  ...
  return (
    <form
      onSubmit={async (e) => {
        e.preventDefault();
        try { await api.post("/newsletter", { email }); setDone(true); } catch (_) {}
      }}
